...
TEMPLATES = {
    ...
    "utb-report": PPTTemplate(
        name="UTB Report",
        min_slides=3,              # Title + at least 2 content slides
        max_slides=8,              # 6 standard + buffer
        min_companies=0,           # Section-based, not company-per-slide
        require_logos=False,
        require_websites=False,
        require_descriptions=False,
        min_words_per_description=30,
        min_content_paragraphs=2,
        require_actionable_content=False,
    ),
}
...
def compare_pptx_to_template(
    ...
    # ==========================================================================
    # UTB REPORT: TABLE AND CHART VALIDATION
    # ==========================================================================

    if template.name == "UTB Report":
        # Check that at least one table exists on content slides (slides 2-5)
        total_checks += 1
        tables_found = 0
        for slide in slides:
            slide_num = slide.get("number", 0)
            if 2 <= slide_num <= 5:
                if slide.get("table_count", 0) > 0 or slide.get("has_table", False):
                    tables_found += 1
                # Fallback: check if "table" mentioned in shape types
                for shape in slide.get("shapes", []):
                    if isinstance(shape, dict) and shape.get("type") == "table":
                        tables_found += 1
                        break
        ...
        if tables_found == 0:
            discrepancies.append(Discrepancy(
                ...
                location="Slides 2-5",
                expected="At least 1 table (Business Overview, Past M&A, or Target List)",
                actual=f"{tables_found} tables found on content slides",
                suggestion="Check generatePresentation() in server.js — table generation may be failing silently.",
            ))
        else:
            passed_checks += 1

        # Check for empty content slides (< 30 words)
        total_checks += 1
        empty_slides = []
        for slide in slides:
            slide_num = slide.get("number", 0)
        ...
        if empty_slides:
            discrepancies.append(Discrepancy(
                severity=Severity.HIGH,
                category="empty_content_slides",
                location=f"Slides {', '.join(str(s) for s in empty_slides)}",
                expected="Each content slide should have ≥30 words",
                actual=f"{len(empty_slides)} slides have <30 words",
                suggestion="Check data pipeline — synthesis may return empty results for some sections.",
            ))
        else:
            passed_checks += 1
