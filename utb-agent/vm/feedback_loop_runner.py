...
# UTB has all logic in a single server.js file
SCOPE_FILES = {
    "research": "server.js",
    "synthesis": "server.js",
    "layout": "server.js",
    "formatting": "server.js",
    "mixed": "server.js",
}
SCOPE_DONT_TOUCH = {
    "research": "",
...
SCOPE_WHAT_TO_CHANGE = {
    "research": (
        "CHANGE TYPE: Perplexity query prompts + API response parsing\n"
        "WHERE: backend/utb/server.js — buildResearchQueries() function\n"
        "Fix the search query text or response parsing logic.\n"
        "DO NOT add hardcoded fallback data."
    ),
    "synthesis": (
        "CHANGE TYPE: ChatGPT synthesis prompts + JSON schema\n"
        "WHERE: backend/utb/server.js — synthesize*() functions\n"
        "Fix the synthesis prompt text or JSON schema validation.\n"
        "DO NOT add hardcoded fallback content."
    ),
    "layout": (
        "CHANGE TYPE: pptxgenjs slide generation logic\n"
        "WHERE: backend/utb/server.js — generatePresentation() function\n"
        "Fix table dimensions, chart params, slide element positioning.\n"
        "Match YCP theme: headers #1524A9, labels #011AB7, company #007FFF."
    ),
    "formatting": (
        "CHANGE TYPE: Colors, fonts, positions in pptxgenjs calls\n"
        "WHERE: backend/utb/server.js — generatePresentation() function\n"
        "Fix font (Segoe UI 14pt), colors (see YCP theme), element positions.\n"
        "Header lines at y:1.02 (thick) and y:1.10 (thin)."
    ),
    "mixed": (
        "CHANGE TYPE: Mixed — issues span research, synthesis, or slides\n"
        "WHERE: backend/utb/server.js — trace from buildResearchQueries() "
        "through synthesize*() to generatePresentation().\n"
