...
function trackOpenAICost(model, inputTokens, outputTokens, feature = 'unknown') {
  ...
  if (totalCost > 0.01) {
    console.log(
      `[Cost] ${feature} | ${model} | $${totalCost.toFixed(4)} (${inputTokens}/${outputTokens} tokens)`
    );
  }
...
async function filterVerifiedWebsites(companies) {
  ...
  for (let i = 0; i < companies.length; i += batchSize) {
    ...
    console.log(
      `  Verified ${Math.min(i + batchSize, companies.length)}/${companies.length}. Working: ${verified.length}`
    );
  }

  console.log(
    `Website verification done in ${((Date.now() - startTime) / 1000).toFixed(1)}s. Working: ${verified.length}/${companies.length}`
  );
  return verified;
}
...
// Expand regional inputs to individual countries using AI (V5/V6 version)
async function expandRegionToCountriesV5(regionInput) {
  ...
  const isRegion =
    regionKeywords.some((keyword) => inputLower.includes(keyword)) && !inputLower.includes(','); // If comma-separated, user already specified countries
      ...
      batch.map(async (company) => {
        try {
          ...
          if (!pageContent || pageContent.length < 100) {
            console.log(
              `    ✗ REMOVED: ${company.company_name} (website inaccessible or insufficient content)`
            );
...
app.post('/api/find-target-v5', async (req, res) => {
  ...
  res.json({
    success: true,
    message:
      'Request received. Agentic deep search running. Results will be emailed in ~15-25 minutes.',
  });
  ...
  try {
    ...
    const expandedCountry = await expandRegionToCountriesV5(Country);
    ...
    const needWebsite = uniqueCompanies.filter(
      (c) => !c.website || c.website === 'unknown' || !c.website.startsWith('http')
    );
...
Only include real company websites (not LinkedIn, Facebook, directories). If you can't find a website, omit that company.`;
        ...
        if (jsonMatch) {
          ...
          if (Array.isArray(parsed.websites)) {
            for (const w of parsed.websites) {
              const original = needWebsite.find(
                (c) =>
    ...
    const validationResults = await validateCompaniesV5(
      preFiltered,
      Business,
      expandedCountry,
      Exclusion
    ...
    for (let i = 0; i < companyList.length; i += batchSize) {
      ...
      const batchResults = await Promise.all(
        batch.map(async (companyName) => {
          ...
          console.log(
            `  Fetched pageText for ${companyName}: value=${pageText === null ? 'NULL' : typeof pageText === 'string' ? `string(${pageText.length})` : typeof pageText}`
          );
  ...
  for (let stepIdx = 0; stepIdx < filteringSteps.length; stepIdx++) {
    ...
    if (keptCompanies.length >= 3) {
      ...
      if (removedCompanies.length > 0) {
        ...
        sheetData.push([
          'OUT OF SCOPE - Not matching: "' + filterCriteria + '"',
          'Business Description (Reason for Exclusion)',
        ]);
  ...
  try {
    ...
    for (const company of companies) {
      ...
      const removeCompanySuffix = (name) => {
        ...
        return name
          .replace(
            /\s*(Co\.,?\s*Ltd\.?|Ltd\.?|Sdn\.?\s*Bhd\.?|Pte\.?\s*Ltd\.?|Inc\.?|Corp\.?|LLC|GmbH|JSC|PT\.?|Tbk\.?|S\.?A\.?|PLC)\s*$/gi,
            ''
          )
          .trim();
      };
      ...
      const cleanLocationValue = (location, label) => {
        ...
        if (cleaned.includes('{') && cleaned.includes('}')) {
          try {
            // Try to extract the value from JSON-like string
            const match =
              cleaned.match(/"HQ"\s*:\s*"([^"]+)"/i) || cleaned.match(/"([^"]+)"\s*:\s*"([^"]+)"/);
            if (match) {
      ...
      if (validBreakdownItems.length >= 2) {
        ...
        const textContent = validBreakdownItems
...
Return ONLY valid JSON.`;
    ...
    if (!result) {
      console.log(
        '  AI review agent (both Gemini 3 Flash and GPT-4o) returned empty, keeping original data'
      );
      return companyData;
    }
    ...
    for (let i = 0; i < websites.length; i++) {
      ...
      try {
        ...
        if (missingFields.length > 0 && basicInfo.company_name) {
          console.log(
            `  Step 5: Searching online for missing mandatory info: ${missingFields.join(', ')}...`
          );
        ...
        let companyData = {
          ...
          breakdown_title:
            ensureString(productsBreakdown.breakdown_title) || 'Products and Applications',
        ...
        console.log(
          `  ✓ Completed: ${companyData.title || companyData.company_name} (${companyData.key_metrics?.length || 0} metrics after review)`
        );
...
IMPORTANT: If you cannot find EXACT percentages with sources, return empty arrays. No approximations allowed.`).catch(
...
REMEMBER: Concrete facts only. No generic statements. If you cannot provide specific evidence, do not include the capability.`).catch(
...
IMPORTANT: List at least 10 competitors. This should be a comprehensive view of the competitive landscape, not a shortlist.`).catch(
      (e) => ({ section: 'competitors', error: e.message })
    ),
...
REMEMBER: An MD reading this should deeply understand HOW this company thinks about M&A.`).catch(
      (e) => ({ section: 'ma_analysis', error: e.message })
    ),
...
IMPORTANT: These must be REAL companies, not hypothetical descriptions. If you cannot identify 10 real companies, include as many as you can find with confidence.`).catch(
      (e) => ({ section: 'ideal_target', error: e.message })
    ),
  ];
  ...
  const styleDataRow = (row, isAlternate = false) => {
    row.eachCell((cell) => {
      cell.fill = {
        type: 'pattern',
        pattern: 'solid',
        fgColor: { argb: isAlternate ? lightGray : white },
      };
      cell.font = { size: 10 };
...
app.post('/api/utb', async (req, res) => {
  ...
  try {
    ...
    await sendEmail(
      ...
      {
        content: excelBase64,
        name: `UTB_${companyName.replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().toISOString().split('T')[0]}.xlsx`,
      }
    );
...
// Report component configurations for M&A buy-side DD
const DD_COMPONENTS = {
  executive_summary: {
    title: 'Executive Summary',
    analysisPrompt: `Key highlights, deal thesis, critical findings`,
    reportSection: `<h2>Executive Summary</h2>
  ...
  company_profile: {
    ...
    analysisPrompt: `Company name, HQ location, founding year, industry/sector, brief history, key milestones`,
    reportSection: `<h2>Company Profile</h2>
  ...
  business_model: {
    ...
    analysisPrompt: `Products/services offered, revenue streams, how they make money, target customers, geographic markets`,
    reportSection: `<h2>Business Model</h2>
    - Products & services
    - Revenue model
    - Customer segments
    - Geographic presence`
  ...
  market_analysis: {
    ...
    analysisPrompt: `TAM/SAM/SOM, market growth trends, key drivers and headwinds, regulatory environment`,
    reportSection: `<h2>Market Analysis</h2>
    - Market size & growth
    - Key trends
  ...
  customers: {
    ...
    analysisPrompt: `Customer segments, concentration (top customers %), retention/churn, contract terms, relationships`,
    reportSection: `<h2>Customer Analysis</h2>
    - Customer segments
    - Concentration risk
    - Retention metrics
    - Key relationships`
  ...
  competition: {
    ...
    analysisPrompt: `Direct/indirect competitors, market positioning, barriers to entry, competitive advantages, threats`,
    reportSection: `<h2>Competitive Landscape</h2>
    - Key competitors
    - Positioning
    - Competitive moat
    - Threats`
  ...
  management: {
    ...
    analysisPrompt: `Key executives and backgrounds, ownership structure, board composition, key person dependencies`,
    reportSection: `<h2>Management Team</h2>
    - Leadership team
    - Track record
    - Ownership structure`
  },
  operations: {
    title: 'Operations',
    analysisPrompt: `Facilities/locations, employee count, org structure, technology/systems, suppliers, processes`,
    reportSection: `<h2>Operations</h2>
  ...
  risks: {
    ...
    analysisPrompt: `Business risks, customer concentration, key person risk, competitive threats, operational issues, financial concerns, regulatory risks, any deal-breakers`,
    reportSection: `<h2>Risks & Red Flags</h2>
    - Critical concerns
    - Business risks
    - Operational risks
    - Market risks`
  ...
  opportunities: {
    ...
    analysisPrompt: `Geographic expansion, new products/services, cross-sell/upsell, synergy potential, M&A opportunities`,
    reportSection: `<h2>Growth Opportunities</h2>
  ...
  gaps: {
    ...
    analysisPrompt: `Missing critical information, data that needs verification, unanswered questions, recommended follow-ups`,
    reportSection: `<h2>Information Gaps & Next Steps</h2>
    - Missing information
    - Items to verify
  ...
  meeting_notes: {
    title: 'Meeting Notes',
    analysisPrompt: `Meeting context (attendees, date, purpose), key discussions, commitments made, action items, follow-up questions`,
    reportSection: `<h2>Meeting Notes</h2>
    - Key takeaways
...
async function generateDueDiligenceReport(
  ...
  instructions,
  reportLength,
  components = ['executive_summary', 'company_profile', 'business_model', 'risks', 'gaps']
) {
  console.log(`\n${'='.repeat(60)}`);
  ...
  if (instructions) {
    ...
    for (const url of urls.slice(0, 5)) {
      try {
        console.log(`[DD] Fetching URL: ${url}`);
        const scraped = await scrapeWebsite(url);
        if (scraped.success) {
          onlineResearchContent += `\n\n=== ONLINE: ${scraped.url} ===\n${scraped.content.substring(0, 20000)}\n`;
        }
      } catch (e) {
        console.log(`[DD] URL fetch failed: ${e.message}`);
      }
    }
  }
  ...
  const componentPrompts = selectedComponents.map((c, i) => {
...
- Be thorough but focused on the requested sections`;
  ...
  const reportPrompt = `You are a professional M&A report writer. Using the analysis below, generate a Due Diligence Report.
...
REPORT SECTIONS TO INCLUDE (in this order):
${sectionList}
...
app.post('/api/due-diligence', async (req, res) => {
  const {
    ...
    audioFiles = [],
    instructions,
    components = ['executive_summary', 'company_profile', 'business_model', 'risks', 'gaps'], // selected report sections
    reportLength,
  ...
  try {
    ...
    for (const audioFile of audioFiles) {
      ...
      if (result.error) {
        ...
        console.log(
          `[DD] Transcribed ${audioFile.name}: ${result.text.substring(0, 100)}... (${result.language})`
        );
        transcripts.push({
          name: audioFile.name,
          text: result.text,
    ...
    const combinedTranscript = translatedTranscripts
      .map(
        (t) =>
    ...
    if (outputType === 'transcript') {
      ...
      files.forEach((f, i) =>
        console.log(`[DD]   File ${i + 1}: ${f.name} (${f.type}) - ${f.content?.length || 0} chars`)
      );
      ...
      outputContent = await generateDueDiligenceReport(
        allFiles,
        instructions,
        length,
        components
      ...
      emailSubject = `Due Diligence Report (${sectionCount} sections) - ${files[0]?.name || audioFiles[0]?.name || 'Analysis'}`;
...
`;
    ...
    console.log(
      `[DD] Attachments: ${attachments.map((a) => a.filename || a.name || 'unnamed').join(', ')}`
    );
      ...
      if (isJsonMessage) {
        ...
        if (data.type === 'start') {
          ...
          const detectLanguageFromText = (text) => {
            ...
            // Check for Vietnamese (special diacritics - using Unicode ranges)
            if (/[\u00e0-\u00ff\u0100-\u017f\u1ea0-\u1ef9]/.test(text)) return 'vi';
