...
describe('ai-models', () => {
  describe('MODEL_CONFIG', () => {
    it('has configuration for gpt-5.1', () => {
      expect(MODEL_CONFIG['gpt-5.1']).toBeDefined();
      expect(MODEL_CONFIG['gpt-5.1'].provider).toBe('openai');
      expect(MODEL_CONFIG['gpt-5.1'].cost.input).toBeGreaterThan(0);
      expect(MODEL_CONFIG['gpt-5.1'].cost.output).toBeGreaterThan(0);
    });
  ...
  describe('calculateModelCost', () => {
    it('calculates gpt-5.1 cost correctly', () => {
      const { cost, breakdown } = calculateModelCost('gpt-5.1', 1000000, 500000);
      expect(breakdown.input).toBe(1.25); // 1M * $1.25/1M
    ...
    it('handles zero tokens', () => {
      const { cost } = calculateModelCost('gpt-5.1', 0, 0);
      expect(cost).toBe(0);
    });
  });
  ...
  describe('createCostTracker', () => {
    it('tracks costs across multiple calls', () => {
      ...
      tracker.add('gpt-5.1', 1000, 1000, 'search');
    ...
    it('groups by model and feature', () => {
      const tracker = createCostTracker();
      tracker.add('gpt-5.1', 1000, 500, 'search');
      tracker.add('gpt-5.1', 2000, 1000, 'search');
      tracker.add('gemini-2.5-flash', 1000, 500, 'validation');
      const summary = tracker.getSummary();
      expect(Object.keys(summary.byModel)).toContain('gpt-5.1');
    ...
    it('returns total cost correctly', () => {
      const tracker = createCostTracker();
      const cost1 = tracker.add('gpt-5.1', 1000000, 0);
      const cost2 = tracker.add('gpt-5.1', 0, 1000000);
