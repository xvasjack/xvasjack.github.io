// Quality gates — check functions that run BETWEEN pipeline stages.
...
function hasMeaningfulContentValue(value, depth = 0) {
  if (depth > 7) return false;
  if (value == null) return false;
  if (typeof value === 'string') return isMeaningfulText(value);
  if (typeof value === 'number') return Number.isFinite(value);
  if (typeof value === 'boolean') return true;
  if (Array.isArray(value)) return value.some((item) => hasMeaningfulContentValue(item, depth + 1));
  if (typeof value !== 'object') return false;

  for (const [k, v] of Object.entries(value)) {
    if (k.startsWith('_')) continue;
    if (['key', 'title', 'name', 'section', 'type', 'dataType'].includes(k)) continue;
    if (hasMeaningfulContentValue(v, depth + 1)) return true;
  }
  return false;
}
...
function blockHasRenderableData(block) {
  ...
  if (Object.prototype.hasOwnProperty.call(block, 'content')) {
    ...
    if (hasEmptyPlaceholderText(content) || hasTruncationArtifactText(content)) return false;
  }
  ...
  return candidateFields.some((field) => hasMeaningfulContentValue(field));
}
...
function validateResearchQuality(researchData) {
  ...
  // Industry-universal regex: matches company names with common suffixes across all industries
  const companyRegex =
    /[A-Z][a-z]+ (?:Corp|Ltd|Inc|Co|Group|Energy|Electric|Power|Solutions|Technologies|Pharma|Health|Financial|Capital|Holdings|Partners|Systems|Services|Global|International|Industries|Consulting|Medical|Bank|Insurance|Logistics|Telecom|Motors|Aviation|Semiconductor|Ventures|Labs)/;
  ...
  // Metric 6: Numeric data density (10 pts) — research with specific numbers is stronger
  const numberRegex = /\d+(?:\.\d+)?(?:\s*%|\s*(?:billion|million|thousand|B|M|K))/gi;
  const numberMatches = allContent.match(numberRegex) || [];
  const numberScore = Math.min(10, Math.round((numberMatches.length / 5) * 10));
  if (numberMatches.length < 3) {
    issues.push(
      `Only ${numberMatches.length} numeric data points found (need >= 3 for strong research)`
    );
  }

  const score = charScore + topicScore + structuredScore + companyScore + yearScore + numberScore;

  // Effective max is 85 (30+25+0+10+10+10). Pass at 50 requires at least chars+topics+one quality signal.
  return {
    pass: score >= 50,
    score,
    issues,
    ...
    for (const subKey of expectedSubKeys) {
      ...
      for (const item of players) {
        if (item?.profile && !item?.revenue) {
          failures.push(
            `Competitors: ${subKey} player "${item.name || 'unknown'}" has nested profile but no flat revenue — data may be lost in building`
          );
        }
      }
    }
...
// Single-country synthesis check
...
function validateSingleCountrySynthesis(synthesis, industry) {
  ...
  if (cp && typeof cp === 'object') {
    ...
    // Content check: check competitor descriptions for specific metrics
  ...
  if (Array.isArray(insights)) {
    ...
    // Timing check: timing field must contain a year or quarter
...
function validatePptData(blocks) {
  ...
  // This checker is used in both full-deck and section-level flows.
  // For section-level check, require at least one buildable block.
  ...
  for (const block of blocks) {
    if (block?.type === 'company' || block?.type === 'competitor') {
      ...
      if (hasEmptyPlaceholderText(desc) || hasTruncationArtifactText(desc)) {
        emptyBlocks.push(
  ...
  for (const block of blocks) {
    if (block?.chartData?.series) {
      ...
      if (block.chartData.stacked || block.chartType === 'stackedBar') {
        ...
        if (negativeCount > 0) {
          chartIssues.push(
            `Block "${block.title || 'unknown'}": ${negativeCount} negative values in stacked chart data — building will be incorrect`
          );
        }
      }
    }
  ...
  // Check thin/placeholder content ratio
  const thinContentCount = blocks.filter((b) => !blockHasRenderableData(b)).length;
  const thinContentRatio = thinContentCount / blocks.length;
  if (thinContentRatio >= 0.4) {
    emptyBlocks.push(
      `${Math.round(thinContentRatio * 100)}% of blocks are thin/placeholder (threshold < 40%)`
    );
  }
