...
const {
  truncate,
  truncateSubtitle: _truncateSubtitle,
  fitTextToShape,
  safeArray,
...
const { validatePptData, blockHasRenderableData } = require('./content-gates');
const {
...
const { applyTemplateClonePostprocess } = require('./template-fill');
const {
  isTransientKey,
...
const RENDER_COMPACTION_MODE = String(process.env.RENDER_COMPACTION_MODE || 'off')
  .trim()
  .toLowerCase();
...
const TABLE_CELL_HARD_CAP = envNumber('TABLE_CELL_HARD_CAP', 3000, {
  min: 800,
  max: 12000,
});
...
// Only enforces the 3000-char hard cap for file safety (XML node size).
// No cosmetic truncation — content is preserved in full. maxLen parameter is ignored
// because callers historically passed cosmetic limits (15-260 chars) that silently
// destroyed AI-generated research content.
function safeCell(value, _maxLen) {
  ...
  if (str.length > hardCap) {
    ...
    // Inline truncation — file safety cap, not content preference.
...
function getRenderArrayLimit(pathKey) {
  if (RENDER_COMPACTION_MODE === 'off') return Number.POSITIVE_INFINITY;
  if (STRICT_TEMPLATE_FIDELITY) return Number.POSITIVE_INFINITY;
  const key = ensureString(pathKey).toLowerCase();
...
function compactRenderString(pathKey, value) {
  const str = ensureString(value).replace(/\s+/g, ' ').trim();
  if (!str) return '';
  // No cosmetic truncation in the real build path. Warn instead of silently chopping content.
  if (RENDER_COMPACTION_MODE === 'off' || STRICT_TEMPLATE_FIDELITY) return str;
...
function compactRenderPayload(node, path = []) {
  ...
  if (Array.isArray(node)) {
    ...
    if (Number.isFinite(maxItems) && maxItems < node.length) {
      console.warn(
        `[PPT-CONTENT-SIZE] Array "${pathKey}" has ${node.length} items (budget ${maxItems}). Preserving all items — layout may need adjustment.`
      );
    }
...
// Build-time transient key detection delegated to ./cleanup-temp-fields.js (canonical module).
...
// Default is warn-only to avoid dropping an otherwise strong run due to new non-template keys.
const STRICT_RENDER_NORMALIZATION = !/^(0|false|no|off)$/i.test(
  String(process.env.STRICT_RENDER_NORMALIZATION || 'false').trim()
);
...
function normalizeMarketForRender(rawSection) {
  ...
  // are absent, so strict build normalization does not reject otherwise valid
...
async function auditGeneratedPptFormatting(pptxBuffer, { strictMode = false } = {}) {
  ...
  try {
    ...
    for (const partName of layoutAndMasterFiles) {
      ...
      // Collect all line Y offsets across all layouts/masters.
      // Using only the first source is brittle and can trigger false-positive drift.
      for (const match of partXml.matchAll(/<a:off x="0" y="(\d+)"/g)) {
        lineYCandidates.push({ source: partName, y: Number(match[1]) });
      }
    }
    ...
    // Y-position drift checks use nearest line candidates across all layout/master sources.
    if (lineYCandidates.length > 0) {
      ...
      const nearest = (target) => {
        if (!lineYCandidates.length) return null;
        let best = lineYCandidates[0];
        let delta = Math.abs(best.y - target);
        for (const candidate of lineYCandidates.slice(1)) {
        ...
        return { y: best.y, delta, source: best.source };
      };
      ...
      checks.headerFooterY = {
        ...
        source: {
          top: top?.source || null,
          bottom: bottom?.source || null,
          footer: footer?.source || null,
        },
      };

      // Layout-level line drift guard should only block on material, visually significant shifts.
      // Fine-grained drift is enforced by the per-slide runInfo below.
      const severeLayoutDriftEmu = 120000; // ~0.13in
      if (
        (top?.delta || 0) > severeLayoutDriftEmu ||
        (bottom?.delta || 0) > severeLayoutDriftEmu ||
        (footer?.delta || 0) > severeLayoutDriftEmu
      ) {
        ...
        if (top?.delta > 500) {
          layoutDriftDetails.push({
            ...
            actualY: top.y,
            deltaEmu: top.delta,
            severity: top.delta > severeLayoutDriftEmu ? 'error' : 'info',
          });
        }
        if (bottom?.delta > 500) {
          layoutDriftDetails.push({
            ...
            severity: bottom.delta > severeLayoutDriftEmu ? 'error' : 'info',
          });
        }
        if (footer?.delta > 500) {
          layoutDriftDetails.push({
            ...
            severity: footer.delta > severeLayoutDriftEmu ? 'error' : 'info',
          });
        }
        issues.push({
          severity: 'critical',
    ...
    // Per-slide header/footer drift scan using dedicated runInfo module
    try {
      ...
      // Write drift report to disk for runInfo
      try {
...
/**
 * Compute a 0-100 fit score predicting how well rows will build in expectedRect.
 * Used to choose fallback building strategy before attempting table build.
...
function shouldAllowCompetitiveOptionalGroupGap(sectionName, pptGate, blocks) {
  ...
  const buildableGroups = new Set();
  for (const block of blocks) {
    const groupKey = String(block?.key || '').trim();
    if (!groupKey) continue;
    if (blockHasRenderableData(block)) buildableGroups.add(groupKey);
  }

  return buildableGroups.has('localMajor') && buildableGroups.has('foreignPlayers');
}
...
async function generateSingleCountryPPT(synthesis, countryAnalysis, scope) {
  ...
  const buildNormalizationWarnings = [];
  const buildNormalizationErrors = [];
  {
    ...
    if (policyNormalized.droppedKeys.length > 0) {
      ...
      buildNormalizationWarnings.push(msg);
      buildNormalizationErrors.push(msg);
    }
    ...
    if (unsupportedMarketKeys.length > 0) {
      const msg = `[market] unsupported build keys after normalization: ${unsupportedMarketKeys.join(', ')}`;
      buildNormalizationWarnings.push(msg);
      buildNormalizationErrors.push(msg);
    }
  ...
  if (STRICT_RENDER_NORMALIZATION && buildNormalizationErrors.length > 0) {
    const errorSummary = buildNormalizationErrors.slice(0, 6).join(' | ');
    throw new Error(`Build normalization rejected non-template/transient keys: ${errorSummary}`);
  }
  // Build compaction: keep deck layout readable and stable even when synthesis is verbose.
  ...
  // Apply check, deduplication, and description enrichment to all player arrays
  ...
  if (buildNormalizationWarnings.length > 0) {
    console.warn(`[PPT NORMALIZE] ${buildNormalizationWarnings.join(' | ')}`);
  }

  // Title text passes through; pptxgenjs fit:'shrink' handles overflow.
  // Only the 15,000-char HARD_TEXT_CAP in ensureString/truncate guards file safety.
  ...
  function addSlideWithTitle(title, subtitle = '', options = {}) {
    ...
    // No cosmetic subtitle truncation — fit:'shrink' handles overflow.
    const subtitleText = ensureString(subtitle).trim();
  ...
  // Rule: margins >2 are interpreted as points and converted to inches.
  ...
  function compactTableRowsForDensity(
    ...
    // Never truncate cell content — only collect overflow info for downstream rethink/font scaling.
    const compactedRows = rows.map((row, rowIdx) => {
      ...
      return row.map((cell, colIdx) => {
        ...
        if (typeof cell === 'string') {
          ...
          cellLen = ensureString(cell.text, '').length;
        } else {
          cellLen = ensureString(cell, '').length;
        }
        if (cellLen > cap) {
          overflowSamples.push({
            ...
            cap,
            len: cellLen,
            severe: cellLen > Math.round(cap * 2.2),
            sample: ensureString(typeof cell === 'object' ? cell.text : cell, '').substring(0, 120),
          });
        }
        return cell;
      });
    ...
    if (overflowSamples.length > 0) {
      const preview = overflowSamples
        ...
        .join(' | ');
      console.warn(
        `[PPT-CONTENT-SIZE] ${context}: ${overflowSamples.length} table cell(s) exceed density budget (${preview}). Content preserved — font scaling or rethink may apply.`
      );
    }
    return { rows: compactedRows, changed: 0, overflowSamples };
  }
  ...
  function safeAddTable(slide, rows, options = {}, context = 'table') {
    ...
    if (addOptions && typeof addOptions === 'object') {
      ...
      if (hadAutoPage) {
        console.log(`[PPT] ${resolvedContext}: stripped autoPage flags for deterministic building`);
      }
    ...
    if (shouldAlignToTemplate) {
      ...
      if (expectedTableRect) {
        ...
        // geometry in extracted metadata. In that case keep builder geometry without
        // treating it as a hard-styleMatch failure.
        console.warn(
          `[PPT TEMPLATE] ${resolvedContext}: no explicit table geometry for template slide ${activeTemplateContext.slideNumber}; using builder geometry`
        );
      }
    }
    if (addOptions && typeof addOptions === 'object') {
    ...
    if (fitResult.recommendation === 'fallback') {
      if (strictGeometryMode) {
        console.warn(
          `[STRICT GEOMETRY] ${resolvedContext}: table fit score=${fitResult.score} (${fitResult.recommendation}) is weak. Applying controlled fallback to keep delivery path alive while preserving core content.`
        );
      }
      console.warn(
        `[PPT] ${resolvedContext}: table fit score=${fitResult.score} (${fitResult.recommendation}) — ${
          strictGeometryMode ? 'keeping full table content' : 'applying aggressive recovery'
        }`
      );
      if (!strictGeometryMode) {
          ...
          tableRows.push([
            {
              ...
              options: {
                colspan: countTableColumns(tableRows) || 1,
                italic: true,
                color: '999999',
              },
            },
      ...
      if (strictGeometryMode) {
        console.warn(
          `[STRICT GEOMETRY] ${resolvedContext}: table fit score=${fitResult.score} (${fitResult.recommendation}) is weak. Applying controlled row/column reduction instead of failing run.`
        );
      }
      console.log(
        `[PPT] ${resolvedContext}: table fit score=${fitResult.score} (${fitResult.recommendation}) — ${
          strictGeometryMode ? 'keeping full table content' : 'applying truncation'
        }`
      );
      if (!strictGeometryMode) {
        if (tableRows.length > TABLE_FLEX_MAX_ROWS) {
    ...
    // Absolute hard cap to prevent pathological payloads from breaking table rendering.
    // Keep this high so normal strategic content is preserved.
    for (let ri = 0; ri < tableRows.length; ri++) {
      if (!Array.isArray(tableRows[ri])) continue;
      for (let ci = 0; ci < tableRows[ri].length; ci++) {
        const cell = tableRows[ri][ci];
        if (typeof cell === 'string' && cell.length > TABLE_CELL_HARD_CAP) {
          console.warn(
            `[PPT-SIZE-OVERFLOW] ${resolvedContext}: table cell r${ri}c${ci} exceeded hard cap (${cell.length} > ${TABLE_CELL_HARD_CAP}); clipping for file safety`
          );
          tableRows[ri][ci] = cell.slice(0, Math.max(0, TABLE_CELL_HARD_CAP - 3)) + '...';
        } else if (
          cell &&
          typeof cell === 'object' &&
          typeof cell.text === 'string' &&
          cell.text.length > TABLE_CELL_HARD_CAP
        ) {
          console.warn(
            `[PPT-SIZE-OVERFLOW] ${resolvedContext}: table cell r${ri}c${ci} exceeded hard cap (${cell.text.length} > ${TABLE_CELL_HARD_CAP}); clipping for file safety`
          );
          cell.text = cell.text.slice(0, Math.max(0, TABLE_CELL_HARD_CAP - 3)) + '...';
        }
      }
    }
    if (addOptions && typeof addOptions === 'object') {
          ...
          if (severe.length > 0) {
            if (strictGeometryMode) {
              const preview = severe
                ...
                .join(', ');
              console.warn(
                `[STRICT GEOMETRY] ${resolvedContext}: ${severe.length} severe cell(s) still exceed density budget after rethink (${preview}). Keeping full cell text to avoid silent content loss.`
              );
              templateUsageStats.tableFallbacks.push({
                key: resolvedContext,
                reason: `density-overflow-untrimmed: ${severe.length} severe cells`,
                recoveryType: 'density-overflow-untrimmed',
                severeCells: severe.length,
              });
            } else {
              // Warn about dense cells but preserve full content.
              // The TABLE_CELL_HARD_CAP guard above handles true file-safety limits.
              const preview = severe
                .slice(0, 4)
                .map((entry) => `r${entry.row}c${entry.col}:${entry.len}>${entry.cap}`)
                .join(', ');
              console.warn(
                `[PPT-CONTENT-SIZE] ${resolvedContext}: ${severe.length} severe cell(s) exceed density budget after rethink (${preview}). Preserving full content — TABLE_CELL_HARD_CAP (${TABLE_CELL_HARD_CAP}) guards file safety.`
              );
              templateUsageStats.tableFallbacks.push({
                key: resolvedContext,
                reason: `density-overflow-preserved: ${severe.length} severe cells`,
                recoveryType: 'density-overflow-untrimmed',
                severeCells: severe.length,
              });
            }
  ...
  // Hardening switches: template-first building for production decks.
  ...
  const EMPTY_PLACEHOLDER_TEXT_PATTERNS = [
  ...
  function hasEmptyPlaceholderText(value) {
    const text = ensureString(value).replace(/\s+/g, ' ').trim();
    if (!text) return true;
    return EMPTY_PLACEHOLDER_TEXT_PATTERNS.some((re) => re.test(text));
  }
  ...
  function hasMeaningfulNarrative(value, minWords = 6) {
    const text = ensureString(value).replace(/\s+/g, ' ').trim();
    if (!text) return false;
    if (hasEmptyPlaceholderText(text) || hasTruncationArtifactText(text)) return false;
  ...
  function sanitizeSectionPayload(value, depth = 0) {
    ...
    if (typeof value === 'string') {
      if (hasTruncationArtifactText(value) || hasEmptyPlaceholderText(value)) return '';
      return value;
    }
  ...
  function generateMarketChartSlide(slide, block) {
    ...
    if (hasChartSeries || hasChartValues) {
      ...
      // No chart data - build text insights as structured content blocks
      if (insights.length > 0) {
        ...
        // Last resort: scan ALL string fields in data for anything buildable
        const fallbackTexts = [];
  ...
  function collectMarketInsights(key, data) {
    ...
    switch (key) {
      case 'tpes':
        ...
        // keyInsight built in callout overlay, not duplicated here
  ...
  // Hard-fail on build errors so malformed slides never reach output.
  function generatePatternSlide(block) {
    ...
    try {
      const buildMarketBlock = () => {
        generateMarketChartSlide(slide, block);
        addMarketSubTable(slide, block);
      };
      const builderRegistry = {
        // Policy
        foundationalActs: () => buildFoundationalActs(slide, block.data),
        nationalPolicy: () => buildNationalPolicy(slide, block.data),
        investmentRestrictions: () => buildInvestmentRestrictions(slide, block.data),
        keyIncentives: () => buildKeyIncentives(slide, block.data),
        regulatorySummary: () => buildFoundationalActs(slide, block.data),
        // Market
        marketSizeAndGrowth: buildMarketBlock,
        supplyAndDemandDynamics: buildMarketBlock,
        supplyAndDemandData: buildMarketBlock,
        pricingAndTariffStructures: buildMarketBlock,
        pricingAndEconomics: buildMarketBlock,
        pricingAndCostBenchmarks: buildMarketBlock,
        infrastructureAndGrid: buildMarketBlock,
        tpes: buildMarketBlock,
        finalDemand: buildMarketBlock,
        electricity: buildMarketBlock,
        gasLng: buildMarketBlock,
        pricing: buildMarketBlock,
        escoMarket: buildMarketBlock,
        // Competitors
        ...
        caseStudy: () => buildCaseStudy(slide, block.data),
        maActivity: () => buildMAActivity(slide, block.data),
        // Depth
        dealEconomics: () => buildDealEconomics(slide, block.data),
        escoEconomics: () => buildDealEconomics(slide, block.data), // legacy key
        entryStrategy: () => buildEntryStrategy(slide, block.data),
        implementation: () => buildImplementation(slide, block.data),
        targetSegments: () => buildTargetSegments(slide, block.data),
        // Summary
        goNoGo: () => buildGoNoGo(slide, block.data),
        opportunitiesObstacles: () => buildOpportunitiesObstacles(slide, block.data),
        keyInsights: () => buildKeyInsights(slide, block.data),
        timingIntelligence: () => buildTimingIntelligence(slide, block.data),
        lessonsLearned: () => buildLessonsLearned(slide, block.data),
      };
      const builder = builderRegistry[block.key];
      if (typeof builder !== 'function') {
        const knownKeys = Object.keys(builderRegistry).sort().join(', ');
        throw new Error(
      ...
      // Recovery: add placeholder instead of crashing — let remaining slides build
      addDataUnavailableMessage(slide, 'Content building failed for this section');
    }
  ...
  // Builds any dynamic/unknown block by detecting the data shape and choosing
  ...
  function buildGenericContentSlide(slide, block) {
    const data = block.data;
    ...
    console.log(`  [PPT] Generic build for "${block.key}" (type: ${detectedType})`);

    // Route to existing specialized builders where data shape matches
    switch (detectedType) {
      case 'company_comparison': {
        // Delegate to the company slide builder
      ...
      case 'composition_breakdown': {
        // Delegate to market chart builder
        generateMarketChartSlide(slide, block);
        return;
      }

      case 'opportunities_vs_barriers': {
        buildOpportunitiesObstacles(slide, data);
        return;
      }

      case 'case_study': {
        buildCaseStudy(slide, data);
        return;
      }
    ...
    // Generic building: extract structured content from the data
    let currentY = CONTENT_Y;
    ...
    // 5. Last resort: scan ALL remaining string fields for buildable content
    if (currentY <= CONTENT_Y + 0.1) {
      // Nothing was built yet — extract any text from data
      const fallbackTexts = [];
      for (const [fk, fv] of Object.entries(data)) {
        if (typeof fv === 'string' && fv.length > 20 && fv.length < 3000) {
          fallbackTexts.push(ensureString(fv));
  ...
  function buildFoundationalActs(slide, data) {
  ...
  function buildNationalPolicy(slide, data) {
  ...
  function buildInvestmentRestrictions(slide, data) {
  ...
  function buildKeyIncentives(slide, data) {
  ...
  function buildCaseStudy(slide, data) {
    ...
    // Use addCaseStudyRows pattern for rich building
    const caseRows = [
  ...
  function buildMAActivity(slide, data) {
    const deals =
  ...
  function buildDealEconomics(slide, data) {
  ...
  function buildEntryStrategy(slide, data) {
    ...
    if (harvey.criteria && Array.isArray(harvey.criteria) && harvey.criteria.length > 0) {
      ...
      harvey.criteria.forEach((crit, idx) => {
        ...
        harveyKeys.slice(0, 3).forEach((key) => {
          ...
          row.push({ text: buildHarvey(harvey[mappedKey], idx) });
        });
        harveyRows.push(row);
      });
  ...
  function buildImplementation(slide, data) {
  ...
  function buildTargetSegments(slide, data) {
  ...
  function buildGoNoGo(slide, data) {
  ...
  function buildOpportunitiesObstacles(slide, data) {
  ...
  function dynamicText(text, maxChars, baseFontPt, floorPt) {
    ...
    // Preserve full content at minimum font size. Never silently truncate.
    // pptxgenjs fit:'shrink' or downstream layout will handle overflow visually.
    return { text: ensureString(text), fontSize: minPt };
  }

  function buildKeyInsights(slide, data) {
  ...
  function buildTimingIntelligence(slide, data) {
  ...
  function buildLessonsLearned(slide, data) {
  ...
  function generateSection(sectionName, sectionNumber, totalSections, sectionData) {
    ...
    if (!sectionHasContent(blocks)) {
      ...
      // Section has no real content - build one summary slide instead of hollow slides
    ...
    for (const block of blocks) {
      ...
      const isLikelyEmpty =
        ...
        hasEmptyPlaceholderText(blockDataStr) ||
  ...
  for (const sec of sectionConfigs) {
    try {
      ...
      if (sec.name === 'Policy & Regulatory' && policy?.regulatorySummary) {
        ...
        if (regRows.length > 0) {
          // Always build with the template's horizontal flow geometry for policy consistency.
          addHorizontalFlowTable(regSummarySlide, regRows, { font: FONT });
        } else if (typeof regData === 'object' && Object.keys(regData).length > 0) {
          // Keep policy building template-anchored even for malformed structures.
          const fallbackRows = [];
          ...
          if (fallbackRows.length > 0) {
            ...
            addDataUnavailableMessage(
              regSummarySlide,
              `${country} regulatory summary not buildable in table format`
            );
          }
        }
      }
  ...
  if (contentSlides.length > 0) {
    ...
    for (const sl of contentSlides) {
      ...
      if (hasEmptyPlaceholderText(slideText) || hasTruncationArtifactText(slideText)) {
        emptySlideCount++;
      }
    }
  ...
  if (templateUsageStats.slideRenderFailures.length > 0) {
    ...
    if (strictGeometryMode) {
      throw new Error(
        `[STRICT GEOMETRY] Hard fail: ${templateUsageStats.slideRenderFailures.length} slide building failure(s) for blocks: ${failKeys}. Any build failure is a hard fail in strict geometry mode.`
      );
    }
    console.warn(
      `[PPT TEMPLATE] Recoverable building failures (${templateUsageStats.slideRenderFailures.length}/${totalResolvedBlocks}): ${failKeys}`
    );
    ...
    if (templateUsageStats.slideRenderFailures.length > totalResolvedBlocks * 0.5) {
      throw new Error(
        `PPT building failures too severe (${templateUsageStats.slideRenderFailures.length}/${totalResolvedBlocks}): ${failKeys}`
      );
    }
  }
  ...
  if (geometryIssues.length > 0) {
    if (strictGeometryMode || STRICT_TEMPLATE_FIDELITY) {
      throw new Error(
        `Template styleMatch violation: geometry issues (${geometryIssues.length}): ${geometryIssues.slice(0, 10).join(', ')}`
      );
    }
    console.warn(
  ...
  if (formattingAudit.warningCount > 0) {
    ...
    if (strictGeometryMode) {
      ...
      // Extract blocking slide keys from each warning issue for precise runInfo.
  ...
  if (relIntegrity.missingInternalTargets.length > 0) {
    const examples = relIntegrity.missingInternalTargets
      ...
      .join(' | ');
    throw new Error(
      `PPT relationship fileSafety failed: ${relIntegrity.missingInternalTargets.length} broken internal target(s); ${examples}`
    );
  }
  if (
    ...
    const examples = relIntegrity.invalidExternalTargets
      ...
      .join(' | ');
    throw new Error(
      `PPT external relationship fileSafety failed: ${relIntegrity.invalidExternalTargets.length} invalid external target(s); ${examples}`
    );
  }
  if (relIntegrity.checkedInternal > 0) {
    console.log(
      `[PPT] Relationship fileSafety check passed (${relIntegrity.checkedInternal} targets)`
    );
  }
  ...
  // Attach run metrics for runInfo/quality gating in server pipeline.
