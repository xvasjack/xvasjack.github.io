...
describe('parseDealEconomics', () => {
  test('parses a complete deal economics object', () => {
    const dealEcon = {
      ...
      contractTerms: { duration: '5 years', revenueSplit: 'Client 70% / Provider 30%', guaranteeStructure: 'Performance' },
  ...
  test('detects contradictory growth claims', () => {
    const synthesis = {
      marketOpportunityAssessment: {
        growthTrajectory: 'The energy services market is growing at 15% CAGR driven by policy mandates',
      },
...
describe('getDecisionScore', () => {
  test('scores high-quality synthesis above threshold', () => {
    const synthesis = {
      ...
      marketOpportunityAssessment: {
        totalAddressableMarket: '$90M calculated from 1,200 factories at $500K average spend with 15% savings potential. This represents a significant opportunity because industrial energy costs are rising 8% annually, resulting in increased demand for efficiency services. Schneider Electric entered in 2018 and should prioritize expanding beyond Bangkok.',
      },
      competitivePositioning: {
        keyPlayers: [
          { name: 'ENGIE Corp', description: '$2.3B revenue, 18% market share, entered Thailand in 2015 via JV with B.Grimm Power Ltd. Focus on industrial parks, 12 active contracts averaging $2M each. Therefore their weakness is limited provincial network coverage.' },
        ],
      },
      keyInsights: [
        {
    ...
    expect(result.flagged.length).toBeLessThan(5);
  });
...
describe('analyze', () => {
  test('returns comprehensive report for good synthesis', () => {
    const synthesis = {
      ...
      keyInsights: [
        { title: 'Cost pressure insight', data: '8% annual wage increase 2021-2024', pattern: 'Aging workforce', implication: 'Target CFOs', timing: 'Q2 2026' },
      ],
      depth: {
        dealEconomics: {
          typicalDealSize: { min: '$2M', max: '$10M', average: '$5M' },
...
describe('sparse and noisy payload handling', () => {
  ...
  test('handles noisy JSON with extra fields', () => {
    const synthesis = {
      ...
      randomField: [1, 2, 3],
      executiveSummary: ['Valid paragraph with Schneider Electric Corp reaching $180M revenue in 2024, growing at 12% annually. This represents a compelling opportunity because industrial demand is rising.'],
      depth: {
        dealEconomics: {
...
describe('extractClaims', () => {
  test('extracts growth claims from text', () => {
    const text = 'The solar market is growing at 20% annually. Battery storage demand increased by 35%.';
