...
const CFG_REVIEW_DEEPEN_MAX_ITERATIONS = parseBoundedIntEnv('REVIEW_DEEPEN_MAX_ITERATIONS', 2, 3);
const CFG_REVIEW_DEEPEN_TARGET_SCORE = parsePositiveIntEnv('REVIEW_DEEPEN_TARGET_SCORE', 75);
const CFG_REFINEMENT_MAX_ITERATIONS = parseBoundedIntEnv('REFINEMENT_MAX_ITERATIONS', 2, 3);
const CFG_MIN_CONFIDENCE_SCORE = parsePositiveIntEnv('MIN_CONFIDENCE_SCORE', 80);
const CFG_FINAL_REVIEW_MAX_ITERATIONS = parseBoundedIntEnv('FINAL_REVIEW_MAX_ITERATIONS', 2, 3);
...
const CFG_SYNTHESIS_TOPIC_MAX_CHARS = parseBoundedIntEnv('SYNTHESIS_TOPIC_MAX_CHARS', 1800, 2400);
...
function hasCorruptedPayloadMarkers(value, depth = 0) {
  if (depth > 8 || value == null) return false;
  if (typeof value === 'string') {
    return containsPlaceholderText(value) || hasTruncationArtifact(value);
  }
  if (Array.isArray(value)) {
    return value.some((item) => hasCorruptedPayloadMarkers(item, depth + 1));
  }
  if (typeof value !== 'object') return false;
  return Object.entries(value).some(([key, child]) => {
    if (String(key || '').startsWith('_')) return false;
    return hasCorruptedPayloadMarkers(child, depth + 1);
  });
}
...
function isMeaningfulValue(value, depth = 0) {
  if (depth > 8 || value == null) return false;
  ...
  if (Array.isArray(value)) {
    return value.some((item) => isMeaningfulValue(item, depth + 1));
  }
  if (!isPlainSectionObject(value)) return false;
  return Object.values(value).some((child) => isMeaningfulValue(child, depth + 1));
}
...
function mergeSectionValuesPreferRich(existingValue, incomingValue, depth = 0) {
  ...
  if (Array.isArray(existingValue) && Array.isArray(incomingValue)) {
    const existingUseful = existingValue.filter((item) => isMeaningfulValue(item, depth + 1));
    const incomingUseful = incomingValue.filter((item) => isMeaningfulValue(item, depth + 1));
    return incomingUseful.length >= existingUseful.length ? incomingValue : existingValue;
  }

  if (isMeaningfulValue(incomingValue, depth + 1)) return incomingValue;
  if (isMeaningfulValue(existingValue, depth + 1)) return existingValue;
...
async function identifyResearchGaps(synthesis, country, _industry, baselineCodeGate = null) {
  ...
  const compactSnapshot = {
    policy: summarizeForSummary(synthesis?.policy || {}, 'policy', 3500),
    market: summarizeForSummary(synthesis?.market || {}, 'market', 3500),
    competitors: summarizeForSummary(synthesis?.competitors || {}, 'competitors', 3500),
    depth: summarizeForSummary(synthesis?.depth || {}, 'depth', 3000),
    summary: summarizeForSummary(synthesis?.summary || {}, 'summary', 3000),
    existingCheckScores: baselineCodeGate?.scores || synthesis?.contentCheck?.scores || null,
  };
  ...
  try {
    ...
    if (isJsonTruncated(jsonStr)) {
      ...
      try {
        ...
        // Fall through and throw original parse error for clearer runInfo.
      }
    }
...
function validateCompetitorsSynthesis(result) {
  ...
  for (const section of sections) {
    ...
    if (cleanedPlayers.length === 0) {
      warnings.push(`${section}: no viable players after content cleanup`);
    }
  }
...
async function synthesizeWithFallback(prompt, options = {}) {
  ...
  function getRejectReason(acceptResult) {
    if (acceptResult === false || acceptResult == null) return 'content gate returned false';
    ...
    if (typeof acceptResult === 'object') {
      if (acceptResult.pass === false) {
        return (
          ...
          'content gate returned pass=false'
        );
      }
  ...
  function applyContentGate(candidate, tierName) {
  ...
  try {
    ...
    try {
      ...
      if (parsed) {
        const accepted = applyContentGate(parsed, 'Tier 1 (Gemini jsonMode)');
        if (accepted) {
      ...
      if (allowRawExtractionFallback) {
        ...
        if (extractResult.status === 'success' && extractResult.data) {
          const accepted = applyContentGate(extractResult.data, 'Tier 2 (extract from raw)');
          if (accepted) {
      ...
      if (allowRawExtractionFallback) {
        ...
        if (allowTruncationRepair && text && isJsonTruncated(text)) {
          ...
          if (repairResult.status === 'success' && repairResult.data) {
            const accepted = applyContentGate(repairResult.data, 'Tier 3 (repaired)');
            if (accepted) {
  ...
  if (allowProTiers) {
    ...
    try {
      ...
      try {
        const parsed = parseJsonResponse(text);
        if (parsed) {
          const accepted = applyContentGate(parsed, 'Tier 4 (GeminiPro jsonMode)');
          if (accepted) {
        ...
        if (allowRawExtractionFallback) {
          const extractResult = extractJsonFromContent(text);
          if (extractResult.status === 'success' && extractResult.data) {
            const accepted = applyContentGate(extractResult.data, 'Tier 4 (extract)');
            if (accepted) {
    ...
    try {
      ...
      if (allowRawExtractionFallback) {
        const extractResult = extractJsonFromContent(text);
        if (extractResult.status === 'success' && extractResult.data) {
          const accepted = applyContentGate(extractResult.data, 'Tier 5 (GeminiPro no-jsonMode)');
          if (accepted) {
        ...
        if (allowTruncationRepair && text && isJsonTruncated(text)) {
          ...
          const repairResult = extractJsonFromContent(repaired);
          if (repairResult.status === 'success' && repairResult.data) {
            const accepted = applyContentGate(repairResult.data, 'Tier 5 (GeminiPro repaired)');
            if (accepted) {
  ...
  for (let attempt = 0; attempt <= MAX_POLICY_RETRIES; attempt++) {
    ...
    let currentResult = await synthesizeWithFallback(currentPrompt, {
      ...
      accept: (candidate) => {
        ...
        if (hasCorruptedPayloadMarkers(normalized)) {
          return {
            pass: false,
...
${antiArraySuffix}`;
    ...
    if (hasCorruptedPayloadMarkers(candidate)) {
      console.warn(
        `  [synthesizeMarket] Attempt ${attempt}: corrupted artifact found after normalization, retrying...`
      );
      if (attempt === MAX_MARKET_RETRIES) {
        console.error(
          '  [synthesizeMarket] All retries exhausted with corrupted artifacts in market payload'
        );
      }
      continue;
    }
...
function summarizeForSummary(synthesis, section, maxChars) {
  ...
  const snippets = [];
  const pushSnippet = (label, value, maxLen = 300) => {
  ...
  const summarizeObject = (obj, parentKey) => {
    ...
    for (const field of preferredFields) {
      ...
      if (raw == null) continue;
      if (typeof raw === 'string' || typeof raw === 'number' || typeof raw === 'boolean') {
        pushSnippet(`${parentKey}.${field}`, raw, 320);
      }
    }
    ...
    if (Array.isArray(obj.keyMetrics) && obj.keyMetrics.length > 0) {
      const metricPreview = obj.keyMetrics
        ...
        .filter(Boolean)
        .join(' | ');
      pushSnippet(`${parentKey}.keyMetrics`, metricPreview, 400);
    }
    ...
    for (const [field, raw] of scalarEntries.slice(0, 5)) {
      if (preferredFields.includes(field)) continue;
      pushSnippet(`${parentKey}.${field}`, raw, 280);
    }
  };
  ...
  for (const [key, value] of Object.entries(synthesis).slice(0, 10)) {
    ...
    if (Array.isArray(value)) {
      const arrPreview = value
        ...
        .filter(Boolean)
        .join(' | ');
      pushSnippet(`${key}[${value.length}]`, arrPreview, 400);
      continue;
    }
    if (typeof value === 'object') {
...
function buildFallbackInsight(country, evidence, idx) {
  ...
  return {
    ...
    timing:
      'Use a 3-month check window, then launch first pilot in months 4-9 after partner and compliance gates are complete.',
  };
}
...
function validateContentDepth(synthesis) {
  ...
  if (totalCompanies > 0 && longDescriptions > 0) {
    console.log(
      `  [Check] Competitors warning: ${longDescriptions}/${totalCompanies} descriptions >60 words (overflow tolerated, consider trimming)`
    );
  }

  // Strategic insights check: check structured fields (data, implication, timing)
  ...
  // Partner descriptions check (from depth.partnerAssessment)
  const rootDepth =
  ...
  const summaryDepth =
    ...
    !Array.isArray(synthesis.summary.depth)
      ? synthesis.summary.depth
      : null;
  // Merge both potential carriers and prefer meaningfully useful values.
  ...
  if (partners.length > 0 && longPartners > 0) {
    console.log(
      `  [Check] Partners warning: ${longPartners}/${partners.length} descriptions >60 words (overflow tolerated, consider trimming)`
    );
  }
  ...
  console.log(
    `  [Check] Policy: ${scores.policy}/100 | Market: ${scores.market}/100 | Competitors: ${scores.competitors}/100 | Summary: ${scores.summary}/100 | Depth: ${scores.depth}/100 | Overall: ${scores.overall}/100`
  );
  if (failures.length > 0) {
    console.log(`  [Check] Failures: ${failures.join('; ')}`);
  }
...
Return ONLY valid JSON with the SAME STRUCTURE as the original.`;
  ...
  try {
    ...
    const preserved = {
      ...
      contentCheck: originalSynthesis.contentCheck,
...
async function researchCountry(country, industry, clientContext, scope = null) {
  ...
  const check = validateContentDepth(countryAnalysis);
  countryAnalysis.contentCheck = check;

  // If check fails badly, attempt re-research for weak sections
  if (!check.valid && check.scores.overall < 30) {
    console.log(
      `  [CONTENT TOO THIN] Score ${check.scores.overall}/100 — attempting re-research...`
    );

    // Build targeted gap queries from failures
    const gaps = {
      criticalGaps: check.failures.map((f) => ({
    ...
    if (additionalData.gapResearch.length > 0) {
      ...
      if (check.scores.competitors < 50) {
      ...
      // Re-validate
      const recheck = validateContentDepth(countryAnalysis);
      countryAnalysis.contentCheck = recheck;

      if (recheck.scores.overall < 25) {
        console.error(
          `  [ABORT] Content still too thin after retry (${recheck.scores.overall}/100). Will not generate hollow PPT.`
        );
        pipelineController.abort();
        countryAnalysis.aborted = true;
        countryAnalysis.abortReason = `Content depth ${recheck.scores.overall}/100 after retry. Failures: ${recheck.failures.join('; ')}`;
        return countryAnalysis;
      }
    }
  }
  ...
  if (!countryAnalysis.aborted) {
    ...
    try {
      while (finalReviewIteration < FINAL_REVIEW_MAX_ITERATIONS) {
        ...
        // If we just fixed sections, enforce two extra clean check passes.
        if (reviewClean) {
  ...
  countryAnalysis.contentCheck = postFinalCodeGate;
...
Do NOT skip this check. If you catch yourself returning JSON without checking word counts and number counts, you're shipping shallow work.`;
  ...
  // Quality score from content check (reviewer removed — content depth validates quality)
  synthesis.qualityScore = countryAnalysis.contentCheck?.scores?.overall || 50;
  synthesis.reviewIterations = 0;

  return synthesis;
}

// ============ SINGLE-COUNTRY SYNTHESIS ENTRY ============
...
async function synthesizeFindings(countryAnalyses, scope) {
  ...
  if (analyses.length === 0) {
    throw new Error('synthesizeFindings requires at least one country analysis');
  }
  if (analyses.length > 1) {
    const kept = analyses[0]?.country || 'unknown';
    const skipped = analyses
      .slice(1)
      .map((ca) => ca?.country || 'unknown')
      .join(', ');
    console.warn(
      `[Synthesis] Single-country mode enabled. Keeping "${kept}" and skipping: ${skipped}`
    );
  }
  return synthesizeSingleCountry(analyses[0], scope);
}
