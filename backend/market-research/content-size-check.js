// Content size check.
// This analyzes payload size/density before building.
// It can optionally compact payloads, but server content-first mode now runs this
// as analysis-only by default to preserve full narrative depth.
...
/**
 * Analyzes a country analysis object and returns a size report
 ...
 */
function analyzeContentSize(countryAnalysis) {
  const fieldBudgets = [];
  const tableDensity = [];
  const chartSanity = [];
  const issues = [];
 ...
 * @param {object} countryAnalysis
 * @param {{ fieldBudgets: Array, tableDensity: Array, chartSanity: Array }} sizeReport
 * @returns {{ payload: object, compactionLog: Array }}
 */
function compactContent(countryAnalysis, sizeReport) {
  const payload = deepClone(countryAnalysis);
  const compactionLog = [];

  // --- Warn about fields over char budget but preserve content ---
  // Previously this silently trimmed AI-generated research text to fit cosmetic budgets.
  // The renderer now uses font scaling and layout flex instead of truncation.
  for (const fb of sizeReport.fieldBudgets) {
    if (!fb.exceeded) continue;
    const fullPath = fb.key ? `${fb.section}.${fb.key}` : fb.section;
    console.warn(
      `[CONTENT-SIZE] Field "${fullPath}" is ${fb.charCount} chars (budget ${fb.limit}). Preserving full content.`
    );
    compactionLog.push({
      section: fb.section,
      key: fb.key,
  ...
  // --- Table rows over max: this is a true structural limit (slide geometry) ---
  // Tables physically cannot render >16 rows on a slide. This is a hard cap, not cosmetic.
  for (const td of sizeReport.tableDensity) {
    ...
    if (arr.length > maxRows) {
      ...
      console.warn(
        `[CONTENT-SIZE] Table at "${fullPath}" has ${arr.length} rows (max ${TABLE_MAX_ROWS}). Capping to ${maxRows} rows for slide geometry.`
      );
...
// ============ RUN CONTENT SIZE CHECK ============

/**
 * Main entry point. Runs content-size analysis and optionally compacts the payload.
 * @param {object} countryAnalysis
 * @param {{ dryRun?: boolean }} [options]
 * @returns {{ report: object, payload: object, compactionLog: Array }}
 */
function runContentSizeCheck(countryAnalysis, options) {
  const { dryRun = false } = options || {};
  const report = analyzeContentSize(countryAnalysis);

  if (!dryRun && (report.risk === 'high' || report.risk === 'medium')) {
    const { payload, compactionLog } = compactContent(countryAnalysis, report);
