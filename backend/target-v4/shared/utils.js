...
function normalizeCompanyName(name) {
  ...
  return name
    .toLowerCase()
    .replace(/\s*(sdn\.?\s*bhd\.?|bhd\.?|pte\.?\s*ltd\.?|ltd\.?|inc\.?|corp\.?|corporation|llc|l\.l\.c\.?|co\.?\s*ltd\.?|gmbh|s\.?a\.?|s\.?r\.?l\.?|jsc|pjsc|plc|tbk|pt\.?\s*tbk\.?)\s*$/gi, '')
...
function normalizeWebsite(url) {
  ...
  return url
    ...
    .replace(/^www\./, '')
    .replace(/\/+$/, '')
    .replace(/\/(home|index|about|contact|products|services|company|main|default|en|id|th|vi|jp|cn|kr|de|fr)\/?$/i, '')
...
function buildExclusionRules(exclusion) {
  ...
  if (lower.includes('large') || lower.includes('big') || lower.includes('mnc') || lower.includes('multinational')) {
...
function detectMeetingDomain(text) {
  ...
  if (/ebitda|revenue|m&a|acquisition|valuation|irr|npv|dcf|wacc|eps|pe ratio|dividend|capex|opex/i.test(lower)) return 'financial';
  if (/contract|nda|liability|indemnity|jurisdiction|clause|arbitration|compliance|litigation|patent|trademark/i.test(lower)) return 'legal';
  if (/clinical|fda|ema|pharmaceutical|patient|diagnosis|therapeutic|dosage|efficacy|trial|drug/i.test(lower)) return 'medical';
  if (/api|infrastructure|deployment|kubernetes|docker|microservice|database|latency|throughput|scalability/i.test(lower)) return 'technical';
  if (/compensation|benefits|hiring|recruitment|onboarding|retention|performance review|salary|bonus/i.test(lower)) return 'hr';
...
function getDomainInstructions(domain) {
  const instructions = {
    financial: 'Preserve financial terms, metrics, and abbreviations exactly. Use standard financial translation conventions.',
    legal: 'Maintain legal precision. Preserve contract terms, clause references, and legal terminology.',
    medical: 'Use approved medical terminology. Preserve drug names, dosages, and clinical terms exactly.',
    technical: 'Keep technical terms, acronyms, and code references in original form. Translate explanations only.',
